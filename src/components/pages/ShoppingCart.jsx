...
const ShoppingCartPage = () => {
    ...
    try {
      ...
      const mapped = res.data.cart.map((item) => {
        ...
        return {
          ...item,
          image: img,
          guitarId: item.guitar_id, // important for updating quantity
          cartId: item.id, // important for removing item
        };
      });
  ...
  // --- Update quantity ---
  const updateQuantity = async (guitarId, change) => {
    try {
      const existingItem = cartItems.find((item) => item.guitarId === guitarId);
      ...
      setCartItems((prevItems) =>
        prevItems.map((item) =>
          item.guitarId === guitarId ? { ...item, quantity: newQty } : item
        )
      );

      await axios.put(
        `http://localhost:5000/cart/${guitarId}`,
        { quantity: newQty },
        { withCredentials: true }
      );
    } catch (err) {
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex items-center border border-secondary/30 rounded-lg">
                          <button
                            onClick={() => updateQuantity(item.guitarId, -1)}
